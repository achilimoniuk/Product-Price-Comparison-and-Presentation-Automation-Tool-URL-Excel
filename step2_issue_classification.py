...
pricesdf['Contract ID from Model N'] = pricesdf['Contract ID from Model N'].astype('str')
pricesdf['Contract ID from Model N'] = pricesdf['Contract ID from Model N'].str.extract('(\d+)')
...
for index in tqdm(pricesdf.index, desc = colored("Update on check:", "red"), colour="green"):
    ...
    # Both None values 
    if pricesdf['Price from Model N'][index] == 'None' and pricesdf['Price from Excel'][index] == 'None':
        pricesdf['Difference type'][index] = 'none value'
        pricesdf['Are prices matched?'][index] = 'no'
        pricesdf['difference'][index] = 'none value'
    # One None Value 
    elif pricesdf['Price from Model N'][index] == 'None' or pricesdf['Price from Excel'][index] == 'None':
        pricesdf['Difference type'][index] = 'none value' 
        pricesdf['Are prices matched?'][index] = 'no'
        pricesdf['difference'][index] = 'none value'
    # Error Value 
    elif pricesdf['Price from Model N'][index] == 'error':
        pricesdf['Difference type'][index] = 'not found in Model N' 
        pricesdf['Are prices matched?'][index] = 'no'
        pricesdf['difference'][index] = 'not found'
    # Both numeric values
    else:
        pricesdf['Price from Model N'][index] =  pd.to_numeric(pricesdf['Price from Model N'][index])
        pricesdf['Price from Excel'][index] = pd.to_numeric(pricesdf['Price from Excel'][index])
        # The same values
        if pricesdf['Price from Model N'][index] == pricesdf['Price from Excel'][index]:
            pricesdf['Difference type'][index] = 'no difference'
            pricesdf['Are prices matched?'][index] = 'yes'
            pricesdf['difference'][index] = '0'
        # Different values
        else: 
            pricesdf['difference'][index] = round(abs(pricesdf['Price from Model N'][index] - pricesdf['Price from Excel'][index]),4)
    ...
    # Contract ID check
    # Both None values
    if pricesdf['Contract ID from Model N'][index] == 'None' and pricesdf['Contract ID from Excel'][index]=='None':
        pricesdf['Is Contract ID the same?'][index] = 'none value'
    # One None value
    elif pricesdf['Contract ID from Model N'][index] == 'None'and pricesdf['Contract ID from Excel'][index] != 'None':
        pricesdf['Is Contract ID the same?'][index] = 'none value'
    # One None value
    elif pricesdf['Contract ID from Model N'][index] != 'None'and pricesdf['Contract ID from Excel'][index] == 'None':
        pricesdf['Is Contract ID the same?'][index] = 'none value excel'
    # Test Load Issue in Contract ID
    elif pricesdf['Contract ID from Model N'][index] == 'TestLoadIssue':
        pricesdf['Is Contract ID the same?'][index] = 'none value'
    # Conract IDs the same
    elif pd.to_numeric(pricesdf['Contract ID from Model N'][index]) == pd.to_numeric(pricesdf['Contract ID from Excel'][index]):
    ...
    # Both None values
    if pricesdf['Price Program from Model N'][index] == 'None' and pricesdf['Price Program from Excel'][index] ==  'None':
        pricesdf['Is Price Program Name the same?'][index] = 'none value'  
    # One None value
    if pricesdf['Price Program from Model N'][index] == 'None' or pricesdf['Price Program from Excel'][index] ==  'None':
        pricesdf['Is Price Program Name the same?'][index] = 'none value'
    # Price Program Names the same
    elif pricesdf['Price Program from Model N'][index] == pricesdf['Price Program from Excel'][index]:
...
for index in tqdm(mismatched.index, desc = colored("Update on type issue check:", "red"), colour="green"):
    ...
    if mismatched['Is Contract ID the same?'][index] == 'no':
        # Contract from Model N is the Price List
        if pd.to_numeric(mismatched['Contract ID from Model N'][index]) in issue_list:
    ...
    #    if pd.to_numeric(mismatched['Contract ID from Model N'][index]) in issue_list and pd.to_numeric(mismatched['Contract ID from Excel'][index]) in issue_list:
...
mismatched.to_csv(f'{path}/files/mismatched prices2 {country}.csv') 
mismatched.to_excel(f'{path}/files/mismatched prices2 {country}.xlsx') 
pricesdf.to_csv(f'{path}/files/all transactions2 {country}.csv') 
pricesdf.to_excel(f'{path}/files/all transactions2 {country}.xlsx') 


# Exporting csv with transactions with different Contract ID
#df_contractid = pricesdf.loc[(pricesdf['Is Contract ID the same?'] == 'no')]
#df_contractid.to_csv(f'{path}/files/contract different.csv')

# Exporting csv with transactions with different Price Program Name
#df_program = pricesdf.loc[(pricesdf['Is Price Program Name the same?'] == 'no')]
#df_program.to_csv(f'{path}/files/program different.csv')
